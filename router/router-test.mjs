...
export default function createRouter(express, db, passport, bcrypt, salt, logger, auth_check_test) {
    const router = express.Router();

    // console.log( `${Object.keys(auth_check_test)}` );
    ...
    router.get('/report', logger, auth_check_test.checkAuthenticated, async (req, res) => {
        res.render('test-report', { title: 'Report', username: 'Andrew' });
    });

    router.get('/export', logger, auth_check_test.checkAuthenticated, async (req, res) => {
        res.render('test-export', { title: 'Export', username: 'Andrew' });
    });

    router.get('/record', logger, auth_check_test.checkAuthenticated, async (req, res) => {
        const query_body = req.body;
        res.render('test-record', { title: 'record', username: 'Andrew' });
    });

    router.get('/login', logger, auth_check_test.checkNotAuthenticated, async (req, res) => {
        return res.render('test-login', { title: 'Login', username: 'Andrew' });
    });

    router.post('/login', logger, auth_check_test.checkNotAuthenticated, passport.authenticate('local', {
        successRedirect: '/test/',
        failureRedirect: '/test/login'
    }));

    // router.post('/logout', logger, auth_check_test.checkAuthenticated, async (req, res) => {
    //     return res.redirect('/test/logout');
    // });

    router.delete('/logout', logger, auth_check_test.checkAuthenticated, async (req, res) => {
        req.logOut((err) => {
            if (err) {
                return next(err);
            }
    ...
    router.get('/register', logger, auth_check_test.checkNotAuthenticated, async (req, res) => {
        return res.render('test-register', { title: 'Register', username: 'Andrew' });
    });

    router.post('/register', logger, auth_check_test.checkNotAuthenticated, async (req, res) => {
        ...
        return res.redirect('/test/login'); // return res.json( { "status": "success", "action": "redirect", "destination": "/test/login" } );
    });
